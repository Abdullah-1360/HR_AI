...
export const api = {
  ...
  router: {
    getOverview: async (): Promise<import('./types').RouterOverviewResponse> => {
      const { data } = await client.get('/router/overview');
      return data;
    },
    getProviders: async (): Promise<import('./types').RouterProvider[]> => {
      const { data } = await client.get('/router/providers');
      return data;
    },
    getModels: async (): Promise<import('./types').RouterModel[]> => {
      const { data } = await client.get('/router/models');
      return data;
    },
    getRequests: async (limit = 50, offset = 0, status?: string): Promise<{ items: import('./types').RouterRequestLog[]; total: number }> => {
      const { data } = await client.get('/router/requests', { params: { limit, offset, status } });
      return data;
    },
    getTierWaterfall: async (): Promise<import('./types').RouterTierWaterfall[]> => {
      const { data } = await client.get('/router/tier-waterfall');
      return data;
    },
  },
};
